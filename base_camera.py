...
class CameraEvent(object):
    ...
    def wait(self):
        ...
        if ident not in self.events:
            ...
            # a threading.Event() and a timestamp
    ...
    def set(self):
        ...
        for ident, event in self.events.items():
            if not event[0].isSet():
                ...
                # if the client's event is already set, it means
                # the client did not process a previous frame;
...
class BaseCamera(object):
    ...
    thread = None
    # current frame is stored here by the background thread
    frame  = None
